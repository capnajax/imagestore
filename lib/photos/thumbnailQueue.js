...
async function loadImageFile(pathname) {
  let date = path.parse(pathname).name;
  if (date < 1000000000000) {
    // if it's in seconds, convert to milliseconds
    date *= 1000;
  }

  let thumbailSpecs = dao.getThumbnailSpecs();
  let imageContent = await fs.readFile(pathname);

  let results = {
    pathname,
    content: imageContent,
    date: path.basename(pathname),
    thumbailSpecs
  };
  return results;
}
